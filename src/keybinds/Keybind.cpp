...
std::string lilac::keyToStringFixed(enumKeyCodes code) {
    switch (code) {
...
std::ostream& lilac::operator<<(std::ostream& stream, Keybind const& bind) {
    return stream << bind.toString();
}

std::ostream& lilac::operator<<(std::ostream& stream, keybind_action_id const& id) {
